...
function validation(){
	...
	if(fname.length <3){
		text="<div class='alert-danger'>First name must be more than 3 characters </div>";
		error_message.innerHTML= text;  
		return false;
	}

	if(regxfname.test(fname) == 0){
		text="<div class='alert-danger'>Only alphabets are allowed in first name</div>";
		error_message.innerHTML= text;
		return false;
	}
    ...
    if(lname.length <3){
		text="<div class='alert-danger'>Last name must be more than 3 characters </div>";
		error_message.innerHTML= text;
		return false;
	}  
	
	if(regxlname.test(lname) == 0){
		text="<div class='alert-danger'>Only alphabets are allowed in last name</div>";
		error_message.innerHTML= text;
		return false;
	}
